...
import { getopt } from '../src/utility.js';
...
describe('getopt', () => {
    test('should parse options without arguments', () => {
        ...
        const optstring = 'ab';
        const result = getopt(argv.length, argv, optstring);
    ...
    test('should handle unknown options', () => {
        ...
        const optstring = 'a';
        throws(() => getopt(argv.length, argv, optstring), /Unknown option: -x/);
    });

    test('should handle missing required arguments', () => {
        const argv = ['node', 'script.js', '-a'];
        const optstring = 'a:';
        throws(() => getopt(argv.length, argv, optstring), /Option -a requires an argument/);
    });
